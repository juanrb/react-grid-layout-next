...
const ReactGridLayout = (properties: Partial<Props>) => {
    ...
    const [layout, setLayout] = useState<Layout>(synchronizeLayoutWithChildren(
        properties.layout || [],
        children,
        cols,
        compactType,
    ...
    useEffect(() => {
        setLayout(synchronizeLayoutWithChildren(
            properties.layout || [],
            children,
            cols,
     ...
     */
    const onDragStartFn: GridItemCallback<GridDragEvent> = (
        properties
    ) => {
     ...
     * @param {Element} node The current dragging DOM element
     */
    const onDragFn: GridItemCallback<GridDragEvent> = (
        properties
    ) => {
        const l = getLayoutItem(layout, properties.i);
        if (!l) return;
    ...
    const onResizeFn: GridItemCallback<GridResizeEvent> = (
        properties
    ) => {
        ...
        const [newLayout, l] = withLayoutItem(layout, properties.i, l => {
            ...
            if (preventCollision && !allowOverlap) {
                const collisions = getAllCollisions(layout, { ...l, w: properties.x, h: properties.y }).filter(
    ...
    return (
        ...
        >
            {
                children && (Array.isArray(children) ? React.Children.map(children, child =>
