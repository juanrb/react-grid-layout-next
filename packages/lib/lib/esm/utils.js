...
export function childrenEqual(a, b) {
    return deepEqual(React.Children.map(a, c => c?.["key"]), React.Children.map(b, c => c?.["key"]));
}
...
export function setTransform({ top, left, z, width, height }) {
...
export function setTopLeft({ top, left, z, width, height }) {
    return {
        top: `${top}px`,
        left: `${left}px`,
        width: `${width}px`,
