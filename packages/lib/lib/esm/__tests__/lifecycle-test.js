...
describe("Lifecycle tests", function () {
    ...
    describe("<GridItem >", () => {
        const mockProps = {
            children: _jsx("div", { children: "test child" }),
            cols: 12,
            containerWidth: 1200,
            rowHeight: 300,
            margin: [0, 0],
        ...
        it("Basic Render", () => {
            const wrapper = render(_jsx(GridItem, { ...mockProps }));
        ...
        describe("optional min/max dimension props log err", () => {
            describe("minW", () => {
                ...
                it("2x when string, not number", () => {
                    // $FlowIgnore
                    render(_jsx(GridItem, { ...mockProps, minW: "apple" }));
                    expect(mockError).toHaveBeenCalledTimes(2);
                });
                it("1 err when larger than w prop", () => {
                    render(_jsx(GridItem, { ...mockProps, minW: 400 }));
            ...
            describe("maxW", () => {
                ...
                it("1x when string, not number", () => {
                    // $FlowIgnore
                    render(_jsx(GridItem, { ...mockProps, maxW: "apple" }));
                    expect(mockError).toHaveBeenCalledTimes(1);
                });
                it("1x err when smaller than w prop", () => {
                    render(_jsx(GridItem, { ...mockProps, w: 4, maxW: 2 }));
                    expect(mockError).toHaveBeenCalledTimes(1);
                });
            });
            describe("minH", () => {
                ...
                it("2x when string, not number", () => {
                    // $FlowIgnore
                    render(_jsx(GridItem, { ...mockProps, minH: "apple" }));
                    expect(mockError).toHaveBeenCalledTimes(2);
                });
                it("1x when larger than h prop", () => {
                    render(_jsx(GridItem, { ...mockProps, minH: 200 }));
                    expect(mockError).toHaveBeenCalledTimes(1);
                });
            });
            describe("maxH", () => {
                ...
                it("1x when string, not number", () => {
                    // $FlowIgnore
                    render(_jsx(GridItem, { ...mockProps, maxH: "apple" }));
                    expect(mockError).toHaveBeenCalledTimes(1);
                });
                it("1x when smaller than h prop", () => {
                    render(_jsx(GridItem, { ...mockProps, h: 3, maxH: 2 }));
                    expect(mockError).toHaveBeenCalledTimes(1);
                });
            });
        });
        describe("onDrag", () => {
            it("calls onDragStart prop when droppingPosition prop has expected content", () => {
                ...
                render(_jsx(GridItem, { ...mockProps, 
                    // $FlowIgnore
                    droppingPosition: { left: 1, top: 1, e: {} }, onDragStart: mockFn }));
